...
from lehome_robot_sim_embedding.spline import fit_all_splines
...
def _print_run_summary(output: Path) -> None:
    ...
    print(f"  mean_latent_out_of_range_percent: {float(summary.get('mean_latent_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_latent_out_of_range_percent: {float(summary.get('maximum_latent_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_latent_overshoot_range_fraction: {float(summary.get('maximum_latent_overshoot_std', 0.0)):.6g}")
    print(f"  mean_state_out_of_range_percent: {float(summary.get('mean_state_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_state_out_of_range_percent: {float(summary.get('maximum_state_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_state_overshoot_range_fraction: {float(summary.get('maximum_state_overshoot_std', 0.0)):.6g}")
    print(f"  mean_action_out_of_range_percent: {float(summary.get('mean_action_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_action_out_of_range_percent: {float(summary.get('maximum_action_out_of_range_percent', 0.0)):.4f}")
    print(f"  max_action_overshoot_range_fraction: {float(summary.get('maximum_action_overshoot_std', 0.0)):.6g}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Fit adaptive visual-behavior splines for robot sim embeddings")

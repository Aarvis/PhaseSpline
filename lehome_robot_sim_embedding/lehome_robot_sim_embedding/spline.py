...
def fit_episode_spline(
    ...
    action_transition = np.sqrt(np.mean(np.diff(action_standardized, axis=0) ** 2, axis=1))
    combined_transition = np.maximum.reduce([latent_transition, state_transition, action_transition])
    threshold = float(np.quantile(combined_transition, float(config["mandatory_transition_quantile"])))
    ...
    while True:
        ...
        action_interpolator = _build_interpolator(
            spline_method,
            timestamps[indices],
            action_standardized[indices],
            axis=0,
            cubic_bc_type=cubic_bc_type,
        )
        ...
        reconstructed_action_standardized = action_interpolator(timestamps)
        reconstructed_mean = (
    ...
    maximum_action_ratio = float(action_rmse.max() / max(epsilon_action, 1e-12))
    ...
    payload = {
        ...
        "knot_actions": action[indices].astype(np.float32),
    ...
    result = SplineFitResult(
        ...
        maximum_action_rmse=float(action_rmse.max()),
...
def fit_all_splines(
    ...
    weighted_mean = lambda key: (
        sum(float(item.get(key, 0.0)) * int(item["frames"]) for item in summaries) / max(total_frames, 1)
    )
    max_value = lambda key: max((float(item.get(key, 0.0)) for item in summaries), default=0.0)
    ...
    atomic_json_dump(
        {
            ...
            "maximum_epsilon_ratio": max_value("maximum_epsilon_ratio"),
            "maximum_latent_rmse": max_value("maximum_latent_rmse"),
            "maximum_cosine_distance": max_value("maximum_cosine_distance"),
            "maximum_state_rmse": max_value("maximum_state_rmse"),
            "maximum_action_rmse": max_value("maximum_action_rmse"),
            "mean_latent_out_of_range_percent": weighted_mean("latent_out_of_range_percent"),
            "maximum_latent_out_of_range_percent": max_value("latent_out_of_range_percent"),
            "maximum_latent_overshoot_std": max_value("maximum_latent_overshoot_std"),
            "mean_state_out_of_range_percent": weighted_mean("state_out_of_range_percent"),
            "maximum_state_out_of_range_percent": max_value("state_out_of_range_percent"),
            "maximum_state_overshoot_std": max_value("maximum_state_overshoot_std"),
            "mean_action_out_of_range_percent": weighted_mean("action_out_of_range_percent"),
            "maximum_action_out_of_range_percent": max_value("action_out_of_range_percent"),
            "maximum_action_overshoot_std": max_value("maximum_action_overshoot_std"),

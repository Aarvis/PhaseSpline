...
from .data import ACTION_COLUMN, STATE_COLUMN, VIEW_COLUMNS, MultiViewTransform, discover_episodes
from .model import RobotSimMultiViewVAE
...
) -> tuple[RobotSimMultiViewVAE, dict[str, Any]]:
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    checkpoint_config = checkpoint.get("config", {})
    if "state_dim" not in config["model"]:
        config["model"]["state_dim"] = int(checkpoint_config.get("model", {}).get("state_dim", 12))
    if "action_dim" not in config["model"]:
        config["model"]["action_dim"] = int(checkpoint_config.get("model", {}).get("action_dim", 12))
...
def export_embeddings(
    ...
    transform = MultiViewTransform(int(config["model"]["dino"]["image_size"]))
    ...
    state_dims = [int(value) for value in normalization["state_dimensions"]]
    action_dims = [int(value) for value in normalization["action_dimensions"]]
    image_columns = [VIEW_COLUMNS["top"], VIEW_COLUMNS["left"], VIEW_COLUMNS["right"]]
    ...
    for record in tqdm(episodes, desc="export/episodes", unit="episode"):
        ...
        encoded_images = {view: table[VIEW_COLUMNS[view]].to_pylist() for view in ("top", "left", "right")}
        ...
        with torch.inference_mode():
            for start in frame_progress:
                ...
                images = torch.stack(
                    [
                        torch.stack([transform(encoded_images[view][position]) for view in ("top", "left", "right")])
                        for position in range(start, stop)
                    ]
                ).to(device, non_blocking=True)
                with autocast(device_type=device.type, enabled=amp_enabled, dtype=torch.bfloat16):
                    dino = model.encode_dino(images.flatten(0, 1))
                    views = model.split_multiview_features(dino, (stop - start, 1))
                    posterior = model.posterior_from_views(
                        views.top.patches[:, 0],
